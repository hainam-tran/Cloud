...
{
    class TypicalStudentService
    {
        private static readonly object Lock = new Object();
        private static volatile TypicalStudentService _instance;

        /// <summary>
        /// NewsCategoryFactory singleton object
        /// </summary>
        public static TypicalStudentService Instance
        {
            get
            {
                if (_instance == null)
                {
                    lock (Lock)
                    {
                        if (_instance == null)
                        {
                            _instance = new TypicalStudentService();
        ...
        public void Delete(string languageId, Guid id, out OperationResult operationResult)
        {
            lock (Lock)
            {
                Common.Instance.Delete<TypicalStudent>
                    (obj => obj.TypicalStudentId == id && obj.LanguageId == languageId, out operationResult);
            }
        }

        public TypicalStudent GetById(string languageId, Guid id, out OperationResult operationResult)
        {
            return Common.Instance.SelectFirstOrDefault<TypicalStudent>
                (obj => obj.TypicalStudentId == id && obj.LanguageId == languageId, out operationResult);
        }

        public List<object> GetAll(string languageId, out OperationResult operationResult)
        {
            return Common.Instance.SelectList<TypicalStudent>
        ...
        public List<object> GetPaging(string languageId, int pageNumber, int pageSize, out OperationResult operationResult)
        {
            try
            {
                ...
                return (from obj in entities.TypicalStudent
